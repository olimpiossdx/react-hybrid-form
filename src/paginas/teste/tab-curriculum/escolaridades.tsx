      ...
      <div className="space-y-3">
        {items.map((item, index) => (
          <div
            ...
            <div className="grow grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Instituição</label>
                <input name={`escolaridades[${index}].instituicao`} defaultValue={item.data.instituicao} className="form-input" required />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Curso</label>
                <input name={`escolaridades[${index}].curso`} defaultValue={item.data.curso} className="form-input" required />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Ano Conclusão</label>
                <input
        ...
        {items.length === 0 && (
          <div className="text-center py-6 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg">
            <p className="text-xs text-gray-500 dark:text-gray-400">Nenhuma formação registrada.</p>
          </div>
        )}
      </div>
    </div>
