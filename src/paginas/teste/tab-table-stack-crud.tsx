...
import { Activity, MoreVertical, Save, Shield, User as UserIcon } from 'lucide-react';
  ...
  return (
    <form {...formProps} className="space-y-4">
      ...
      <div>
        ...
        <Autocomplete name="role" label="" options={ROLES_OPTIONS} required className="mb-0" defaultValue={user.role} />
      </div>
      <div className="pt-2">
      ...
      <div className="flex justify-end pt-4 border-t border-gray-200 dark:border-gray-700">
        <button
          type="button"
          onClick={onClose}
          className="px-4 py-2 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white mr-2 transition-colors">
          Cancelar
        </button>
        <button
          type="submit"
          className="flex items-center gap-2 px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold shadow-lg transition-transform active:scale-95">
...
const TabTableStackCRUD = () => {
    ...
    showModal({
      title: (
        <div className="flex items-center gap-2 text-gray-900 dark:text-white">
          <UserIcon className="text-cyan-500" /> Editar Usuário
        </div>
      ),
      size: 'sm',
  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 h-[600px] flex flex-col transition-colors">
      <div className="mb-6 border-b border-gray-100 dark:border-gray-700 pb-4 shrink-0">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-2">
          <UserIcon className="text-purple-600 dark:text-purple-400" />
          Usuários (Modo Stack)
        </h2>
        <p className="text-xs text-gray-500 dark:text-gray-400">Em telas pequenas, as linhas se transformam em cards verticais.</p>
      </div>

      <DataTable.Root instance={table} className="flex-1 overflow-hidden">
        <DataTable.Container>
          {/* AQUI ESTAVA O ERRO: Adicionado o Wrapper <Table> */}
          <DataTable.Table>
            <DataTable.Header>
                ...
                <DataTable.Row key={item.id} className="relative">
                  ...
                  <DataTable.Cell columnIndex={2}>
                    <div className="flex items-center justify-between md:justify-start w-full">
                      ...
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${item.data.active ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800'}`}>
                        <Activity size={12} />
