...
const EmployeeDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    if (response.isSuccess && response.data) setEmployees(response.data);
    setLoading(false);
  };
    ...
    showModal({
      title: (
        <div className="flex items-center gap-2 text-cyan-600 dark:text-cyan-400">
          <Users />{" "}
          <span>{employee ? "Editar Funcionário" : "Novo Funcionário"}</span>
        </div>
      ),
      size: "lg",
      content: ({ onClose }: any) => (
      ...
      <header className="flex justify-between items-end border-b border-gray-200 dark:border-gray-700 pb-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Dashboard administrativo com arquitetura Safe Envelope.
          </p>
        </div>
        <button
