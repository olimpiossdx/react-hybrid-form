...
const EmployeeModal: React.FC<Props> = ({ employee, onSave, onCancel }) => {
  const { formProps, resetSection, setValidators } = useForm<IEmployeeFormData>(
    { id: "employee-modal-form", onSubmit: onSave }
  );
  ...
  useEffect(() => {
    setValidators({
      name: (val: string) =>
        !val || val.length < 3
          ? { message: "Nome completo obrigatório", type: "error" }
          : undefined,
      role: (val: any) =>
        !val ? { message: "Cargo obrigatório", type: "error" } : undefined,
      rating: (val: number) =>
        val < 1 ? { message: "Avaliação mínima 1", type: "error" } : undefined,
    });
  }, [setValidators]);
    ...
    <form {...formProps} className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          <label className="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold mb-1 block">
            Nome
          </label>
          <input
            name="name"
        ...
        <div>
          <label className="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold block mb-1">
            Admissão
          </label>
          <input
            name="admissionDate"
