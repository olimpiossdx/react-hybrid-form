...
const EmployeeFilter: React.FC<{ onFilter: (f: IEmployeeFilter) => void }> = ({ onFilter }) => {
    ...
    <form
      ...
      <div className="flex-1 w-full">
        ...
        <div className="relative">
          <input name="term" className="form-input pl-9" placeholder="Nome ou E-mail..." />
          <Search className="absolute left-2.5 top-2.5 text-gray-400 w-4 h-4 z-10 pointer-events-none" />
        </div>
      </div>
      <div className="w-full md:w-48">
        <Autocomplete name="role" label="Cargo" options={ROLES_OPTIONS} placeholder="Todos" clearable />
      </div>
      <div className="w-full md:w-64">
        <DateRangePicker startDateName="date_start" endDateName="date_end" label="Admissão" matchInputWidth />
      </div>
      <button
        type="submit"
