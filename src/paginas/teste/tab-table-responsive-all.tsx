...
import { Eye, Info, Layout, List, Layers, CreditCard, ChevronDown, Repeat } from 'lucide-react';
        ...
        <div className=\"flex items-start gap-3 flex-1\">
          ...
          <div className=\"flex-1\">
            <h4 className=\"font-semibold text-blue-900 dark:text-blue-100 text-sm mb-1\">
              {title}
              <code className=\"ml-2 text-xs bg-blue-100 dark:bg-blue-900/50 px-2 py-0.5 rounded font-mono\">
                {mode}
              </code>
            </h4>
            ...
            {isExpanded && (
              <div className=\"space-y-3 mt-3 text-sm animate-in fade-in slide-in-from-top-2 duration-300\">
                ...
                <div className=\"pt-2 border-t border-blue-200/50 dark:border-blue-800/50\">
                  <p className=\"font-medium text-blue-900 dark:text-blue-100 mb-1 flex items-center gap-1.5\">
                    🚀 Melhor uso:
                  </p>
          ...
          <div className=\"grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-900/40 rounded-lg border border-gray-100 dark:border-gray-800\">
            <div>
              ...
              <div className=\"space-y-1\">
                <p className=\"text-sm\"><span className=\"text-gray-500\">Email:</span> {row.email}</p>
                <p className=\"text-sm\"><span className=\"text-gray-500\">Telefone:</span> {row.phone}</p>
                <p className=\"text-sm\"><span className=\"text-gray-500\">Localização:</span> {row.location}</p>
              </div>
            </div>
            <div>
              <p className=\"text-xs font-bold text-gray-400 uppercase tracking-wider mb-1\">Metadados</p>
              <div className=\"space-y-1\">
                <p className=\"text-sm\"><span className=\"text-gray-500\">ID:</span> {row.id}</p>
                <p className=\"text-sm\"><span className=\"text-gray-500\">Criado em:</span> {row.createdAt}</p>
                <p className=\"text-sm\"><span className=\"text-gray-500\">Último Login:</span> {row.lastLogin}</p>
              </div>
            </div>
          </div>
        )}
    ...
    <div className=\"p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500\">
      <header className=\"space-y-2 border-b border-gray-200 dark:border-gray-800 pb-6\">
        ...
        <p className=\"text-gray-600 dark:text-gray-400 max-w-3xl leading-relaxed\">
          Exploração completa das estratégias de responsividade baseadas no artigo do Medium. 
          Cada modo resolve um desafio específico de UX em telas pequenas.
        </p>
      </header>
