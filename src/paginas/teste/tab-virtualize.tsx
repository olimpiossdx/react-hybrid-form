...
import { ArrowDownUp, ChevronDown, ChevronUp, Database, RotateCcw, Save, Trash2 } from 'lucide-react';
...
const VirtualListExample = () => {
  ...
  // 1. ESTRUTURA
  const { items: listItems, remove: listRemove, replace: listReplace } = useList(TOTAL_ITEMS);
  ...
  const scrollRefProxy = useMemo(() => ({ current: scrollElement }), [scrollElement]) as React.RefObject<HTMLDivElement>;

  const { virtualItems, scrollToIndex, measureElement, containerProps, wrapperProps } = useVirtualizer({
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 h-[700px] flex flex-col resize-y overflow-hidden transition-colors">
      <div className="flex justify-between items-center mb-4 border-b border-gray-100 dark:border-gray-700 pb-4 shrink-0">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <Database className="text-purple-600 dark:text-purple-400" /> Virtualização Dinâmica
          </h2>
      ...
      <form {...formProps} id="virtual-form" onSubmit={handleSubmit(onSubmit)} className="flex-1 flex flex-col overflow-hidden">
        <div className="grid grid-cols-2 gap-4 mb-4 shrink-0 bg-gray-50 dark:bg-gray-900/30 p-3 rounded border border-gray-200 dark:border-gray-700/50">
          <input name="batchName" className="form-input text-sm" required defaultValue="Lote-Dynamic" />
          <input name="manager" className="form-input text-sm" required placeholder="Responsável..." />
        </div>
                ...
                <div
                  ...
                  <div className="col-span-1 flex justify-center pt-1">
                    <button
                      ...
                      {itemData.isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                    </button>
                  </div>

                  <span className="col-span-2 text-cyan-600 dark:text-cyan-500 font-mono text-xs truncate pt-2">{itemData.sku}</span>
                  ...
                  <div className="col-span-4">
                    ...
                    {itemData.isExpanded && (
                      <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <textarea className="form-input w-full text-xs h-16" defaultValue={itemData.details} />
                      </div>
                    )}
                  </div>
