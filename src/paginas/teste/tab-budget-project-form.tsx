...
const BudgetProjectForm = () => {
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900 p-4 rounded border border-gray-200 dark:border-gray-700">
            ...
            <div className="text-right">
              ...
              <strong
                className={`text-xl ${total - gasto >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
              >
  ...
  const { items, add, remove, replace, clear } = useList<IDespesa>(
    formData.projeto.despesas
  );
  ...
  useEffect(() => {
    setValidators({
      required: (val: any) =>
        !val ? { message: "Obrigatório.", type: "error" } : undefined,
      ...
      validarValorDespesa: (
        ...
        formValues: IProjetoForm
      ) => {
        const totalDisponivel = Number(
          formValues.projeto?.orcamento_total || 0
        );
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 transition-colors max-w-4xl mx-auto">
      ...
      <form
        {...formProps}
        className={
          mode === "carregando" ? "opacity-50 pointer-events-none" : ""
        }
        key={mode}
      >
        {/* PAI: CARD DO PROJETO */}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-3">
            {items.map((item, index) => (
              ...
              >
                <div className="text-gray-400 dark:text-gray-600 font-mono text-xs pt-3 w-6 select-none">
                  #{index + 1}
                </div>

                <div className="grow w-full">
                  <label className="block text-[10px] text-gray-500 dark:text-gray-400 uppercase mb-1">
                    Descrição
                  </label>
                  <input
