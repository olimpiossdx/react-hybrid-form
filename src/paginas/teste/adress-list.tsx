...
const AddressList: React.FC<IAddressListProps> = ({ contactIndex, initialAddresses, isEditing, isDisabled }) => {
  const {
    items: addressFields,
    add: appendAddress,
    remove: removeAddress,
  } = useList<IAddress>(initialAddresses.length > 0 ? initialAddresses : [{ rua: '', cidade: '' }]);
  return (
