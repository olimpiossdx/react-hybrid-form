      ...
      showModal({
        ...
        content: (
          <div className="text-center py-4">
            ...
            <p className="text-sm text-gray-500 mt-2">Um e-mail de confirmação foi enviado para {data.email}.</p>
          </div>
        ),
        actions: (
  ...
  React.useEffect(() => {
    setValidators({
      validarSenha: (val: string) => {
        if (val.length < 6) {
          return { message: 'Mínimo de 6 caracteres', type: 'error' };
        }
      },
      validarConfirmacao: (val: string, _field: FormField | null, formValues: RegistrationFormValues) => {
        if (val !== formValues.pass) {
          return { message: 'As senhas não coincidem', type: 'error' };
        }
      },
    });
  }, [setValidators]);
      ...
      <form {...formProps} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            <input name="name" type="text" className="form-input pl-10" placeholder="Seu nome" required />
        ...
        <div>
          ...
          <div className="relative">
            <input name="email" type="email" className="form-input pl-10" placeholder="seu@email.com" required />
