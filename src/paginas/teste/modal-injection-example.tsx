  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex justify-between mb-8 relative px-4">
        ...
        {steps.map((s, idx) => {
          ...
          return (
            <div
              key={idx}
              className="relative z-10 flex flex-col items-center gap-2 bg-white dark:bg-gray-800 px-2"
            >
              <div
              ...
              <span
                className={`text-[10px] font-bold uppercase ${active ? "text-cyan-600 dark:text-cyan-400" : "text-gray-400"}`}
              >
                {s.title}
              </span>
            </div>
      ...
      <div className="flex justify-between mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
        ...
        <button
          onClick={() =>
            step < 3 ? setStep(step + 1) : (alert("Fim!"), onClose?.())
          }
  ...
  <div className="text-gray-600 dark:text-gray-300 space-y-4">
    ...
    <div className="grid grid-cols-3 gap-4">
      {[1, 2, 3].map((i) => (
        <div
          key={i}
          className="bg-gray-100 dark:bg-gray-800 p-4 rounded text-center font-mono"
        >
          Metric {i}
        </div>
      ))}
...
const ModalInjectionExample = ({
  showModal,
}: {
  showModal: <H, C, A>(opts: IModalOptions<H, C, A>) => void;
}) => {
  // Handlers
  const openLogin = () =>
    showModal({
      ...
      <div className="flex items-center gap-3 mb-6 border-b border-gray-100 dark:border-gray-700 pb-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Teste de injeção de componentes e temas.
          </p>
        </div>
      </div>
