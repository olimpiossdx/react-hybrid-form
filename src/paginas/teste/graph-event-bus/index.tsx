...
import {   AlertTriangle, CheckCircle, Search, Activity,Network, FolderOpen, FileText, ChevronRight, ChevronDown, Cpu, Database, Globe, MousePointer2, Server } from 'lucide-react';
  ...
  return (
    <div className="border-b border-gray-700 p-4 flex justify-between items-center bg-gray-900 rounded-t-lg">
      ...
      <div className="flex gap-2">
        ...
        <button 
          onClick={() => emit('sys:mode_change', { mode: 'emergency', timestamp: Date.now() })}
...
const StatusWidget = ({ id, label }: { id: string, label: string }) => {
  const { on } = useGraph();
  ...
  return (
    <div className={`p-4 rounded border transition-all duration-500 flex flex-col items-center justify-center gap-2 ${isEmergency ? 'bg-red-900/30 border-red-500' : 'bg-gray-800 border-gray-700'}`}>
      {isEmergency ? <AlertTriangle className="text-red-400" /> : <CheckCircle className="text-green-400" />}
      <span className="font-bold text-white text-sm">{label}</span>
      <span className={`text-[10px] uppercase font-mono ${isEmergency ? 'text-red-300' : 'text-gray-500'}`}>
        {status}
      </span>
    </div>
  );
...
const SidebarFilter = () => {
  ...
  return (
    <div className="w-1/3 bg-gray-900/50 p-4 rounded border border-gray-700 flex flex-col gap-2 h-64">
      <h4 className="text-xs font-bold text-gray-500 uppercase">Filtros (Sidebar)</h4>
      <p className="text-[10px] text-gray-400 mb-4">Emite <code>filter:apply</code></p>
      
      <button onClick={() => emit('filter:apply', { category: 'Eletrônicos', term: '' })} className="text-left text-sm text-cyan-400 hover:bg-gray-800 p-2 rounded transition-colors">
         &bull; Eletrônicos
      </button>
      <button onClick={() => emit('filter:apply', { category: 'Livros', term: '' })} className="text-left text-sm text-cyan-400 hover:bg-gray-800 p-2 rounded transition-colors">
         &bull; Livros
      </button>
      <button onClick={() => emit('filter:apply', { category: 'Móveis', term: '' })} className="text-left text-sm text-cyan-400 hover:bg-gray-800 p-2 rounded transition-colors">
         &bull; Móveis
      </button>
    </div>
  );
...
const ContentTable = () => {
  ...
  return (
    <div className="flex-1 bg-gray-800 p-4 rounded border border-gray-700 relative h-64 flex items-center justify-center">
      <div className="absolute top-2 left-3 text-xs font-bold text-gray-500 uppercase">Grid de Dados</div>
      
      {loading ? (
          <span className="text-cyan-400 animate-pulse text-sm">Carregando dados de {filter}...</span>
      ) : (
          <div className="text-center">
              <p className="text-white text-3xl mb-2">📦</p>
              <p className="text-white text-lg">Exibindo: <strong className="text-cyan-400">{filter}</strong></p>
...
const MouseTracker = () => {
    ...
    useEffect(() => {
        return on('mouse:move', ({ x, y }) => {
            ...
            if (targetRef.current) targetRef.current.style.transform = `translate(${x}px, ${y}px)`;
        });
    }, [on]);
    ...
    return (
        ...
        >
            ...
            <div ref={targetRef} className="w-4 h-4 bg-green-500 rounded-full absolute top-0 left-0 -ml-2 -mt-2 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity shadow-[0_0_10px_rgba(34,197,94,0.8)]" />
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none flex-col">
                 <MousePointer2 className="text-gray-600 mb-2" size={32} />
                 <span className="text-gray-500 text-xs">Área de Alta Frequência (60 FPS)</span>
                 <span className="text-gray-600 text-[10px]">Manipulação Direta de DOM</span>
            </div>
        </div>
    );
};
...
const FileNode = ({ node, level = 0 }: { node: TreeNode, level?: number }) => {
    ...
    useEffect(() => {
        return on('tree:search', ({ term }) => {
            if (!term) {
                setIsMatch(false);
                setIsDimmed(false);
                setIsExpanded(level < 1); // Reseta para expandir apenas raiz
                return;
            }
            ...
            // Se estou buscando, expando tudo para mostrar o contexto.
            // Se não sou match, fico meio apagado (dimmed), a menos que seja um container (folder)
            setIsExpanded(true);
            setIsDimmed(!match);
        });
    }, [node.label, on, level]);

    // Se for pasta e estiver "apagada", verificamos se algum filho é match para "acender" a pasta?
    // Para simplificar o exemplo Graph (que é unidirecional), mantemos a lógica local:
    // Se não deu match no nome da pasta, ela fica dimmed, mas os filhos matches brilham.
    ...
    return (
        <div style={{ paddingLeft: level * 16 }}>
            ...
            >
                {node.children ? (
                    <span className="text-gray-500">{isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</span>
                ) : <div className="w-3.5" />}
                
                {node.children ? <FolderOpen size={16} className="text-blue-400" /> : <FileText size={16} className="text-gray-500" />}
                <span>{node.label}</span>
            </div>
            
            {node.children && isExpanded && (
                <div className="border-l border-gray-700 ml-2">
                    {node.children.map(child => <FileNode key={child.id} node={child} level={level + 1} />)}
                </div>
            )}
        </div>
    );
...
const SERVICES = [
    { id: 'svc-1', name: 'API Gateway', type: 'globe', status: 'active', region: 'us-east-1' },
    { id: 'svc-2', name: 'Auth Service', type: 'server', status: 'active', region: 'us-east-1' },
    { id: 'svc-3', name: 'Payment Worker', type: 'cpu', status: 'error', region: 'eu-west-2' },
    { id: 'svc-4', name: 'User DB', type: 'db', status: 'active', region: 'us-east-1' },
];

const ServiceNode = ({ data }: { data: any }) => {
    const { emit } = useGraph(); // Usa 'any' para eventos ad-hoc neste exemplo
    ...
    return (
        <div 
            onClick={() => (emit as any)('service:select', data)}
            className={`
            ...
            `}
        >
            <div className={`p-2 rounded-lg ${data.status === 'error' ? 'bg-red-900/20 text-red-400' : 'bg-gray-700 text-cyan-400'}`}>
                <Icon size={18} />
            </div>
            <div>
...
const ServiceDetails = () => {
    ...
    useEffect(() => {
        // Escuta seleção de qualquer nó da árvore
        return (on as any)('service:select', (data: any) => setSelected(data));
    }, [on]);
    ...
    return (
        <div className="bg-gray-800 border border-gray-600 rounded-lg p-6 h-full animate-in slide-in-from-right-4 fade-in">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-700">
                ...
                <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${selected.status === 'active' ? 'bg-green-900 text-green-400' : 'bg-red-900 text-red-400'}`}>
                    {selected.status}
                </span>
            </div>
            ...
            <div className="space-y-4">
                <div>
                    <label className="text-xs text-gray-500 uppercase">ID do Serviço</label>
                ...
                <div className="p-4 bg-black/30 rounded border border-gray-700 mt-4">
                    ...
                    <div className="h-16 flex items-end gap-1">
                        {[40, 60, 30, 80, 50, 90, 45].map((h, i) => (
                            <div key={i} style={{ height: `${h}%` }} className="flex-1 bg-cyan-900/50 hover:bg-cyan-500 transition-colors rounded-t-sm" />
                        ))}
                    </div>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <section className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                    <span className="bg-purple-900 text-purple-200 text-xs px-2 py-1 rounded border border-purple-700">Cenário 1</span>
                    <h3 className="font-bold text-white text-sm">Broadcast Global (1-para-N)</h3>
                </div>
            ...
            <section className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                    <span className="bg-blue-900 text-blue-200 text-xs px-2 py-1 rounded border border-blue-700">Cenário 2</span>
                    <h3 className="font-bold text-white text-sm">Comunicação Lateral (Irmãos)</h3>
                </div>
            ...
            <section className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                    <span className="bg-yellow-900 text-yellow-200 text-xs px-2 py-1 rounded border border-yellow-700">Cenário 3</span>
                    <h3 className="font-bold text-white text-sm">Busca Profunda em Árvore</h3>
                </div>
                <div className="bg-gray-900/50 p-6 rounded border border-gray-700 h-96 overflow-y-auto custom-scrollbar flex flex-col">
                    ...
                    <div className="pl-1 mt-2 flex-1">
                        {FILE_SYSTEM.map(node => <FileNode key={node.id} node={node} />)}
                    </div>
                </div>
            </section>
            ...
            <section className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                    <span className="bg-cyan-900 text-cyan-200 text-xs px-2 py-1 rounded border border-cyan-700">Cenário 4</span>
                    <h3 className="font-bold text-white text-sm">Navegação Cruzada (Master-Detail)</h3>
                </div>
                <div className="grid grid-cols-2 gap-4 h-96">
                    <div className="bg-gray-900/50 p-4 rounded border border-gray-700 overflow-y-auto space-y-3">
                        {SERVICES.map(svc => <ServiceNode key={svc.id} data={svc} />)}
                    </div>
            ...
            <section className="space-y-4 lg:col-span-2">
                <div className="flex items-center gap-2 mb-2">
                    <span className="bg-green-900 text-green-200 text-xs px-2 py-1 rounded border border-green-700">Performance</span>
                    <h3 className="font-bold text-white text-sm">Zero Re-render (Bypass do React)</h3>
                </div>
                <MouseTracker />
            </section>
        </div>
