...
import { Files, FolderOpen, HardDrive, Image, Paperclip, ShieldCheck } from 'lucide-react';
...
const FileExample = () => {
  ...
  const INITIAL_AVATAR = [{ id: 1, name: 'avatar-atual.png', url: '/img/avatar.png', size: 50000 }];
  ...
  React.useEffect(() => {
    setValidators({
      ...
      validarEdicao: (val: any, field: FormField | null) => validateFileField(val, { required: true }, field) || undefined,
      validarProva: (val: any, field: FormField | null, formValues: any) => {
        if (formValues.has_proof) {
          return validateFileField(val, { required: true }, field) || undefined;
        }
      },
    });
  }, [setValidators]);
    ...
    <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 max-w-5xl mx-auto transition-colors">
      <div className="mb-8 border-b border-gray-100 dark:border-gray-700 pb-4 flex justify-between items-center">
        ...
        <button
          ...
          className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-3 py-1.5 rounded transition-colors">
          Resetar
        </button>
      </div>
      ...
      <form {...formProps} onSubmit={handleSubmit(onSubmit)} className="space-y-10">
        <section>
          <div className="flex items-center gap-2 text-gray-900 dark:text-white font-bold text-sm uppercase border-b border-gray-100 dark:border-gray-800 pb-2">
            <HardDrive size={16} /> 1. Input Nativo
          </div>
          <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded border border-gray-200 dark:border-gray-700">
            ...
            <input
              type="file"
              name="native_file"
              data-validation="validarNativo"
              className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-cyan-50 file:text-cyan-700 hover:file:bg-cyan-100 dark:file:bg-cyan-900 dark:file:text-cyan-300"
            />
          </div>
        </section>
        <section>
          <div className="flex items-center gap-2 text-gray-900 dark:text-white font-bold text-sm uppercase border-b border-gray-100 dark:border-gray-800 pb-2">
            <Image size={16} /> 2. Componente Rico (Single)
          </div>
          <FileInput
            name="avatar"
            label="Foto de Perfil (Required)"
            accept="image/*"
            maxSize={2 * 1024 * 1024}
            required
            defaultValue={INITIAL_AVATAR}
            data-validation="validarAvatar"
          />
        </section>
        <section>
          <div className="flex items-center gap-2 text-gray-900 dark:text-white font-bold text-sm uppercase border-b border-gray-100 dark:border-gray-800 pb-2">
            <Files size={16} /> 3. Galeria (Múltiplos)
          </div>
          <FileInput name="gallery" multiple accept="image/*" maxSize={5 * 1024 * 1024} data-validation="validarGaleria" />
        </section>
        <section>
          <div className="flex items-center gap-2 text-gray-900 dark:text-white font-bold text-sm uppercase border-b border-gray-100 dark:border-gray-800 pb-2">
            <FolderOpen size={16} /> 4. Edição (Misto)
          </div>
          <FileInput
            name="docs_edit"
            label="Documentação"
            multiple
            accept=".pdf, .docx"
            defaultValue={MOCK_SERVER_FILES}
            required
            data-validation="validarEdicao"
          />
        </section>
        <section>
          <div className="flex items-center gap-2 text-gray-900 dark:text-white font-bold text-sm uppercase border-b border-gray-100 dark:border-gray-800 pb-2">
            <ShieldCheck size={16} /> 5. Cruzada
          </div>
        ...
        <div className="flex justify-end pt-4 border-t border-gray-100 dark:border-gray-700">
          <button type="submit" className="px-8 py-3 bg-cyan-600 hover:bg-cyan-700 text-white font-bold rounded-lg shadow-lg">
            Enviar Todos
          </button>
        </div>
      </form>
