...
import { AlertTriangle, Ban, Clock, Database, Edit3, Network, RefreshCw, Send, Trash, XCircle } from 'lucide-react';
...
const ServiceExample: React.FC = () => {
  ...
  const handlePostCreate = async () => {
    ...
    const res = await api.post('https://jsonplaceholder.typicode.com/users', payload);
    if (res.isSuccess) {
  ...
  const handleForce404 = async () => {
    setLoading('404');
    const res = await api.get('https://jsonplaceholder.typicode.com/users/99999', { notifyOnError: true });
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 h-full min-h-[600px] transition-colors">
      ...
      <div className="lg:col-span-5 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Teste de verbos, payloads, erros e resiliência.</p>
        </div>
      ...
      <div className="lg:col-span-7 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden h-[600px] shadow-inner">
        <div className="p-3 bg-gray-100 dark:bg-black/40 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Console de Resposta</span>
          {responseInfo && (
            <span
          ...
          {responseInfo ? (
            <div className="space-y-6">
              <div className="space-y-1">
                <p className="text-gray-400 dark:text-gray-500">// Requisição</p>
                <div className="flex gap-2">
                  <span className="text-purple-600 dark:text-purple-400 font-bold">{responseInfo.method}</span>
                  <span className="text-gray-700 dark:text-gray-300">{responseInfo.title}</span>
                </div>
              </div>
              {responseInfo.payloadSent && (
                <div className="space-y-1">
                  <p className="text-gray-400 dark:text-gray-500">// Payload Enviado</p>
                  <pre className="text-blue-600 dark:text-blue-300">{JSON.stringify(responseInfo.payloadSent, null, 2)}</pre>
                </div>
              )}
              <div className="space-y-1">
                <p className="text-gray-400 dark:text-gray-500">// Resposta da API</p>
                {responseInfo.data ? (
                  <pre className="text-green-600 dark:text-green-300">{JSON.stringify(responseInfo.data, null, 2)}</pre>
                ) : (
              ...
              {responseInfo.error && (
                <div className="space-y-1">
                  <p className="text-gray-400 dark:text-gray-500">// Detalhes do Erro</p>
...
const ApiButton = ({ label, method, icon, onClick, loading, color, subtext, className = '' }: any) => {
  ...
  return (
    <button
      ...
      <div className="mt-0.5">{loading ? <RefreshCw size={16} className="animate-spin" /> : icon}</div>
