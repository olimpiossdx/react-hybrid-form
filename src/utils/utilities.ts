...
export const setNestedValue = (obj: IAnyObject, path: string, value: any): void => {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (isLastKey) { current[key] = value; return; }
    const nextKey = keys[i + 1];
    if (!current[key]) current[key] = !isNaN(Number(nextKey)) ? [] : {};
    current = current[key];
  }
};
...
export const getFormFields = (root: HTMLElement, namePrefix?: string): FormField[] => {
  ...
  return Array.from(nodeList).filter((el): el is FormField => {
    return ((el instanceof HTMLInputElement || el instanceof HTMLSelectElement || el instanceof HTMLTextAreaElement) && el.type !== 'submit' && el.type !== 'button' && el.type !== 'reset' && el.type !== 'image');
  });
};
  ...
  if (field instanceof HTMLInputElement) {
    ...
    if (field.type === 'file') {
        ...
        if (field.dataset.keep) {
             ...
             try { existingFiles = JSON.parse(field.dataset.keep); } catch {}
...
const updateMasterState = (master: HTMLInputElement, form: HTMLElement) => {
  ...
  if (checkedCount === 0) master.checked = false;
  else if (checkedCount === children.length) master.checked = true;
