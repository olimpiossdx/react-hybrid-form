...
export const setNestedValue = (obj: IAnyObject, path: string, value: any): void => {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (!current[key]) current[key] = nextIsNumber ? [] : {};
    current = current[key];
  }
};
...
/**
 * Normaliza o valor.
 * NOVO: Se for Select e tiver data-label, retorna Objeto { value, label }.
 */
...
export const setNativeValue = (element: HTMLElement, value: any) => {
  ...
  // LÓGICA HÍBRIDA: Se recebeu objeto, separa em Value e Label
  if (typeof value === 'object' && value !== null && 'value' in value && 'label' in value) {
      finalValue = value.value;
      element.dataset.label = value.label; // Guarda o contexto no DOM
  } else {
      // Se for valor simples, limpa o label antigo para não sujar o dado
      // (A menos que seja undefined, aí mantemos o que tem)
      if (value !== undefined) {
         delete element.dataset.label;
      }
  }
  
  // Converte para string vazia se for null/undefined para o input
  ...
  if (descriptor && descriptor.set) descriptor.set.call(element, finalValue);
  else (element as any).value = finalValue;
...
export const syncCheckboxGroup = (target: HTMLInputElement, form: HTMLElement) => {
  if (target.dataset.checkboxMaster) {
    ...
    enabledChildren.forEach(child => {
      if (child.checked !== shouldCheckAll) setNativeChecked(child, shouldCheckAll);
    });
  ...
  if (target.name) {
    ...
    if (master) updateMasterState(master, form);
  }
};