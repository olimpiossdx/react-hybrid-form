...
export const validateFile = (file: File, maxSize?: number, accept?: string): string | null => {
  ...
  if (accept) {
    ...
    if (!isValid) {
      return `Tipo inválido. Permitidos: ${accept}`;
    }
  }
  return null;
};
 ...
 */
export const validateFileField = (value: any, rules: FileListRules, field?: HTMLElement | null): string | null => {
  let totalCount = 0;
  let newFiles: File[] = [];
  ...
  if (rules.maxTotalSize) {
    ...
    if (newSize > rules.maxTotalSize) {
      return `Novos arquivos excedem ${formatBytes(rules.maxTotalSize)}.`;
    }
  }
