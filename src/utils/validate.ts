import type { IValidationResult, ValidateFn, FormField } from "../hooks/use-form/props";
...
export const pipe = <T>(...validators: ValidateFn<T>[]): ValidateFn<T> => {
  return (value, field, formValues) => {
    for (const validator of validators) {
      ...
      if (result) return result; // Retorna o erro imediatamente
    }
...
export const required = (msg = "Campo obrigatório") => 
  (value: any, field: FormField | null) => {
      ...
      if (field?.getAttribute('data-has-existing') === 'true') return undefined;
      
      // Regra Padrão
      if (value === null || value === undefined || value === '') return error(msg);
      if (Array.isArray(value) && value.length === 0) return error(msg);
...
export const minLength = (min: number, msg?: string) => 
  (value: string) => {
      if (!value) return undefined; // Deixa o required cuidar de vazio
      if (String(value).length < min) return error(msg || `Mínimo de ${min} caracteres`);
  };

export const isEmail = (msg = "E-mail inválido") => 
  (value: string) => {
      if (!value) return undefined;
      if (!/\S+@\S+\.\S+/.test(value)) return error(msg);
  };
 ...
 */
export const equalsTo = (fieldName: string, msg = "Os campos não conferem") => 
  (value: any, _field: any, formValues: any) => {
      if (value !== formValues[fieldName]) return error(msg);
  };
