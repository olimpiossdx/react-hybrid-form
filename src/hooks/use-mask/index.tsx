...
import { applyCurrencyMask, applyPatternMask } from '../../utils/mask';
...
export const useMask = (format: MaskFormat, options?: UseMaskOptions) => {
  const resolvePattern = (value: string): string => {
    ...
    if (PRESETS[format as string]) {
      return PRESETS[format as string];
    }
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (format === 'currency') {
        maskedValue = applyCurrencyMask(value, options?.currencyOptions?.locale, options?.currencyOptions?.currency);
      } else {
