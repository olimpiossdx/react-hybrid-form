import React, { createContext, useContext, useState, useRef, useCallback, useLayoutEffect } from 'react';
...
const Root: React.FC<TabsRootProps> = ({
  ...
  const handleTabChange = async (newValue: string) => {
    ...
    if (beforeChange) {
      ...
      if (!canChange) return; // Bloqueia navegação
    }
...
const List: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = "" }) => (
  <div className={`flex border-b border-gray-200 dark:border-gray-700 mb-4 gap-2 overflow-x-auto ${className}`}>
    {children}
  </div>
);

const Trigger: React.FC<{ value: string; children: React.ReactNode; className?: string; disabled?: boolean }> = ({
  value, children, className = "", disabled
}) => {
  const context = useContext(TabsContext);
  if (!context) throw new Error("Tabs.Trigger must be used within Tabs.Root");
...
const Content: React.FC<{ value: string; children: React.ReactNode; className?: string }> = ({
  value, children, className = ""
}) => {
  const context = useContext(TabsContext);
  if (!context) throw new Error("Tabs.Content must be used within Tabs.Root");
