...
const Modal: React.FC<IModalProps> = ({ options, onClose }) => {
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      if (options.onClose) {
        options.onClose(); // Callback do usuário
      };
  ...
  const handleBackdropClick = (event: React.MouseEvent) => {
    if (event.target === overlayRef.current && options.closeOnBackdropClick !== false) {
      handleClose();
    };
  };
  ...
  return (<div
      ref={overlayRef} // data-hybrid-modal serve para contarmos quantos modais estão abertos      data-hybrid-modal="true"
