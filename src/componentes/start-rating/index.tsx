...
const StarRating: React.FC<StarRatingProps> = ({
  ...
  useEffect(() => {
    setCurrentValue(Number(startValue));
  }, [startValue]);
  ...
  return (
    <div className={`${effectiveDisabled ? 'opacity-60' : ''} ${className}`}>
      {label && (
          ...
          {starsArray.map((starIndex) => (
            <svg
              ...
              className={`
                  ...
                  ${displayValue >= starIndex ? 'text-yellow-500 dark:text-yellow-400' : 'text-gray-300 dark:text-gray-600'}
              `}
              fill="currentColor"
              viewBox="0 0 20 20">
