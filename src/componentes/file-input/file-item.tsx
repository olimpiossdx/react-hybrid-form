...
export const FileItem: React.FC<IFileItemProps> = ({ name, size, isServer, isNew, onRemove }) => {
  return (
    <li className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg animate-in slide-in-from-top-2 fade-in shadow-sm">
      <div className="flex items-center gap-3 overflow-hidden">
        <File size={18} className={isServer ? 'text-blue-500' : 'text-green-500'} />
      ...
      <button type="button" onClick={onRemove} className="text-gray-400 hover:text-red-500 p-1 z-20 relative">
        <X size={16} />
      </button>
    </li>
  );
