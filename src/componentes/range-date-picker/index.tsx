  ...
  useEffect(() => {
    const handleExternalUpdate = () => {
      if (startInputRef.current) {
        setStart(parseISODate(startInputRef.current.value));
      }
      if (endInputRef.current) {
        setEnd(parseISODate(endInputRef.current.value));
      }
    };
  ...
  const applyRange = (newStart: Date | null, newEnd: Date | null) => {
    ...
    if (endInputRef.current) {
      ...
      if (endInputRef.current.value !== iso) {
        ...
        if (newStart) {
          endInputRef.current.min = toISODate(newStart);
        }
      }
    }
  };
  ...
  const handleVisualBlur = (isStart: boolean) => {
    ...
    if (date) {
      if (!isDateAllowed(date)) {
        ...
        if (isStart) {
          setStartText(toDisplayDate(prev));
        } else {
          setEndText(toDisplayDate(prev));
        }
        return;
      }
      if (isStart) {
          ...
          <div className="w-full sm:w-36 border-b sm:border-b-0 sm:border-r border-gray-200 dark:border-gray-700 p-2 bg-gray-50 dark:bg-gray-900/50 shrink-0 flex flex-row sm:flex-col overflow-x-auto sm:overflow-visible gap-1 no-scrollbar">
            {presets.map((p, i) => (
              <button
                ...
                className="w-auto sm:w-full text-left px-3 py-2 text-xs text-gray-600 dark:text-gray-300 hover:bg-white dark:hover:bg-gray-700 hover:shadow-sm rounded transition-all whitespace-nowrap border border-transparent hover:border-gray-200 dark:hover:border-gray-600">
                {p.label}
              </button>
            ))}
          </div>
