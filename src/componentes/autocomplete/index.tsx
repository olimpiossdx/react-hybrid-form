...
const Autocomplete: React.FC<AutocompleteProps> = ({
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return options.filter((s) => getOptionLabel(s).toLowerCase().includes(inputValue.toLowerCase()));
        ...
        ? filteredSuggestions.map((suggestion, index) => (
            <li
              ...
              className={`
                    ...
                    ${selectedValue === suggestion.value ? 'font-bold bg-gray-100 dark:bg-gray-700' : ''}
                    ${!renderOption ? '' : ''}
                `}
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <div className={`${className}`} ref={containerRef} onBlur={handleBlur}>
      <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300" htmlFor={`autocomplete-${name}`}>
      ...
      <div className="relative">
        ...
        <div className="absolute right-2 top-2.5 text-gray-400 flex items-center">
          {isBusy ? (
            ...
            !disabled &&
            !readOnly && (
              <button type="button" onClick={handleClear} className="hover:text-gray-600 dark:hover:text-white transition-colors">
                ✕
              </button>
            )
          )}
